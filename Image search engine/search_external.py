...
montageA = np.zeros((3006 * 5, 5344, 3), dtype = "uint8")
montageB = np.zeros((3006 * 5, 5344, 3), dtype = "uint8")
...
for j in xrange(0, 10):
	...
	if j < 5:
		montageA[j * 3006:(j + 1) * 3006, :] = result

	# otherwise, the second montage should be used
	else:
		montageB[(j - 5) * 3006:((j - 5) + 1) * 3006, :] = result

# show the results
montageA = cv2.resize(montageA, (340, 750))
montageB = cv2.resize(montageB, (340, 750))
cv2.imshow("Results 1-5", montageA)
cv2.imwrite("SU.png", montageA)
